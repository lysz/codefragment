Ext.onReady(function(){
	...
	var btn03 = Ext.getCmp("t03");   // CP推广上架更新申请(商务)
	var btn04 = Ext.getCmp("t04");   // CP推广测试审核(测试)
	var btn05 = Ext.getCmp("t05");   // CP推广上架审核(运营)
	
	btn03.on('click', appUploadApply);
	btn04.on('click', appUploadApply);
	btn05.on('click', appUploadApply);
    ...
    //--------------------------- Grid panel ---------------------------------------- 
            ...
            columns: [
                ...
                {header: '下架时间', flex: 0.8,  dataIndex: 'offlineDate'},
                {
                    ...
                    items: [{
                        ...
                        tooltip: '编辑',
                        request: 'appupload/updateUploadApply',
                        getClass: function(v, metadata, r, rowIndex, colIndex, store){
                    	    var status = r.get('status');
                    	    //如果app的状态不是'未提交'就不能对它进行修改, 直接隐藏该按钮
                    	    if ('未提交' != status){
                        ...
                        handler: function(grid, rowIndex, colIndex) {
                            ...
                            showWindow('修改', 'appupload/getById', false, params, 'appupload/updateUploadApply');
                        ...
                        iconCls: 'grid-last-column-icon-margin',
                        tooltip: '更新',
                        request: 'appupload/auditUploadApplyForBusiness',
                        getClass: function(v, metadata, r, rowIndex, colIndex, store){
                            var status = r.get('status');
                            //如果app的状态不是'未提交'则就不能对它进行审核操作, 直接隐藏该按钮
                            if (status != '未提交'){
                                return 'x-hide-visibility';
                            }
                        ...
                        handler: function(grid, rowIndex, colIndex) {
                            ...
                            Ext.MessageBox.confirm('提示','你确认更新该数据吗？', function(btn){
                                if(btn != 'yes'){
                                    return;
                                }
                                ...
                                Ext.Ajax.request({
                                    url: 'appupload/auditUploadApplyForBusiness',
                                    method: 'POST',
                                    params: params,
                        ...
                        icon   : 'resources/images/apply_audit.png',
                        //iconCls: 'grid-last-column-icon-margin',
                        request: 'appupload/auditUploadApplyForTest',
                        tooltip: '审核',
                        getClass: function(v, metadata, r, rowIndex, colIndex, store){
                            var status = r.get('status');
                            //如果app的状态不是'待测试审核'就不能对它进行测试审核操作, 直接隐藏该按钮
                            if (status != '待测试审核'){
                                return 'x-hide-visibility';
                            }
                        ...
                        handler: function(grid, rowIndex, colIndex) {
                            ...
                            var params = {
                            	url: 'appupload/auditUploadApplyForTest',
                            	appId: appId,
                            	appName: appName,
                        ...
                        icon   : 'resources/images/apply_audit.png',
                        //iconCls: 'grid-last-column-icon-margin',
                        request: 'appupload/auditUploadApplyForOperation',
                        tooltip: '上架',
                        getClass: function(v, metadata, r, rowIndex, colIndex, store){
                    	    var status = r.get('status');
                    	    //如果app的状态不是'待上架审核'就不能对它进行上架操作, 直接隐藏该按钮
                    	    if (status != '待运营审核'){
                    	    	return 'x-hide-visibility';
                    	    }
                        ...
                        handler: function(grid, rowIndex, colIndex) {
                            ...
                            var params = {
                                url: 'appupload/auditUploadApplyForOperation',
                                appId: appId,
                                appName: appName,
                                companyName: companyName
                            };
     ...
     //-----------------------------------------------------------------------------   
        ...
        function testAudit(params){
        	...
        	var testResultStore = Ext.create('Ext.data.Store', {
                autoLoad: true,
                fields:['dictName', 'dictId'],
                proxy: {
                    type: 'ajax',
