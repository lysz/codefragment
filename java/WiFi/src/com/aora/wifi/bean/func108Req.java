...
public class func108Req extends requestBean{
		...
		if(json!=null && json.size()>0){
			...
			if(data!=null && data.size()>0){
				...
				for(int i=0;i<data.size();i++){
					...
					mp.put("wifi_name", item.containsKey("name")?item.get("name"):"");
					
						
					if(mp.get("wifi_name").toString().equalsIgnoreCase("CMCC")){
						Cmcc=mp;
