		...
		try{
					...
					for(int i=0;i<ls.size();i++){
						...
						if(name.toLowerCase().startsWith("find01")){
							...
							item.put("ptype", "03");
							item.put("link", (map.get("icon")==null?"":map.get("icon").toString()));
							List dt=this.sysAccountDao.getData("SELECT * FROM tbl_find3_news where status=1 order by indate desc limit 4", new String[]{});
							JSONArray darray=new JSONArray();
							if(dt!=null && dt.size()>0){
								for(int j=0;j<dt.size();j++){
									JSONObject ditem=new JSONObject();
							...
							item.put("ptype", "04");
							item.put("link", (map.get("icon")==null?"":map.get("icon").toString()));
							List dt=this.sysAccountDao.getData("SELECT * FROM tbl_find4_video where status=1 order by indate desc limit 3", new String[]{});
							JSONArray darray=new JSONArray();
							if(dt!=null && dt.size()>0){
								for(int j=0;j<dt.size();j++){
									JSONObject ditem=new JSONObject();
							...
							item.put("sortid", (map.get("remark")==null?"":map.get("remark").toString()));							
							item.put("link", (map.get("icon")==null?"":map.get("icon").toString()));
							List dt=this.sysAccountDao.getData("SELECT * FROM tbl_find6_beauties where status=1 order by indate desc limit 2", new String[]{});
							JSONArray darray=new JSONArray();
							if(dt!=null && dt.size()>0){
								for(int j=0;j<dt.size();j++){
									JSONObject ditem=new JSONObject();
							...
							item.put("sortid", (map.get("remark")==null?"":map.get("remark").toString()));							
							item.put("link", (map.get("icon")==null?"":map.get("icon").toString()));
							List dt=this.sysAccountDao.getData("SELECT * FROM tbl_find7_fiction where status=1 order by indate desc limit 4", new String[]{});
							JSONArray darray=new JSONArray();
							if(dt!=null && dt.size()>0){
								for(int j=0;j<dt.size();j++){
									JSONObject ditem=new JSONObject();
...
//							jdata.add(item);													
							...
							item.put("sortid", (map.get("remark")==null?"":map.get("remark").toString()));							
							item.put("link", (map.get("icon")==null?"":map.get("icon").toString()));
							List dt=this.sysAccountDao.getData("SELECT * FROM tbl_find9_app where status=1 order by indate desc limit 5", new String[]{});
							JSONArray darray=new JSONArray();
							if(dt!=null && dt.size()>0){
								for(int j=0;j<dt.size();j++){
									JSONObject ditem=new JSONObject();
		...
		try{
					...
					for(int i=0;i<ls.size();i++){
						...
						if(name.toLowerCase().startsWith("find08")){
							...
							item.put("sortid", (map.get("remark")==null?"":map.get("remark").toString()));							
							item.put("link", (map.get("icon")==null?"":map.get("icon").toString()));
							List dt=this.sysAccountDao.getData("SELECT * FROM tbl_find8_joke where status=1 order by rand() limit 1", new String[]{});
							JSONArray darray=new JSONArray();
							if(dt!=null && dt.size()>0){
								for(int j=0;j<dt.size();j++){
									JSONObject ditem=new JSONObject();
...
public void func351() throws Exception{
    ...
    try{
        ...
        String sql = "SELECT a.id, a.mac_addr, a.content, a.image, a.smallimage, a.praiseQty, a.useraccount, a.groupId, a.loginType, date_format(a.createtime, '%Y-%m-%d %H:%i:%s') as createtime, b.username, b.growth_value, b.image as userimage from tbl_topic a left join tbl_reg_account b on a.useraccount = b.account where a.mac_addr = ? and a.status = 0 order by a.createtime desc limit " + start + ", "+ size;
        ...
        for (int i = 0; i < list.size(); i++){
            ...
            String groupId = map.get("groupId") == null || "".equals(map.get("groupId").toString()) ?  "" : map.get("groupId").toString();
            String loginType = map.get("loginType") == null || "".equals(map.get("loginType").toString()) ?  "" : map.get("loginType").toString();
            ...
            useraccount = StringUtils.isEmpty(groupId) ? useraccount : groupId + ":" +useraccount + ":" + loginType;
...
public void func352() throws Exception{
    ...
    if (StringUtils.isEmpty(ssid) || StringUtils.isEmpty(mac)){
        jsondata.put("resultcode", "-99");
        jsondata.put("msg", "网络不稳定，留言失败");
        respData(jsondata.toString(),request,response);
        return;
    }
    ...
    try{
        ...
        if (useraccount.contains(":") && useraccount.split(":").length > 1){
            groupId = useraccount.split(":")[0];
            loginType = useraccount.split(":")[2];
        ...
        String sql = "INSERT INTO tbl_topic(mac_addr, ssid, content, image, smallimage, status, praiseQty, useraccount, createtime, lat, lng, province, city, district, street, groupId, loginType, uid) values(?, ?, ?, ?, ?, 0, 0, ?, now(), ?, ?, ?, ?, ?, ?, ?, ?,?)";
        sysAccountDao.executedSql(sql, new String[]{mac, ssid, content, image, smallimage, useraccount, lat+"", lng+"", province, city, district, street, groupId, loginType, obj.getUid()});
...
public void func353() throws Exception{
    ...
    try{
        ...
        String topicSQL = "select a.id, a.mac_addr, a.content, a.image, a.praiseQty, a.useraccount, a.groupId, a.loginType, date_format(a.createtime, '%Y-%m-%d %H:%i:%s') as createtime, b.username, b.growth_value, b.image as userimage from tbl_topic a left join tbl_reg_account b on a.useraccount = b.account where a.status = 0 and a.id = ?";
        ...
        String replySQL = "select a.id, b.username, a.useraccount, a.groupId, a.loginType, b.image as userimage, b.growth_value, date_format(a.createtime, '%Y-%m-%d %H:%i:%s') as createtime, a.content,  a.type, (select username from tbl_reg_account where account = substring_index( substring_index(a.touseraccount, ':', 2  ), ':' , -1)) as tousername from tbl_reply a left join tbl_reg_account b on a.useraccount = b.account where a.topicId = ? and a.status = 0 order by createtime desc limit " + start + ", " + size;
...
public void func354() throws Exception{
    ...
    try{
        ...
        if (touseraccount.contains(":") && touseraccount.split(":").length > 1){
            touseraccount = touseraccount.split(":")[1];
        }
        String topicId = obj.getTopicId();
        
        String replySQL = "insert into tbl_reply(content, useraccount, type, topicId, tousername, touseraccount, parentId, status, createtime, groupId, loginType, uid) values(?, ?, ?, ?, ?, ?, ?, 0, now(), ?, ?, ?)";
        
        sysAccountDao.executedSql(replySQL, new String[]{content, useraccount, type, topicId, tousername, touseraccount, to, groupId, loginType, obj.getUid()});
...
public void func356() throws Exception {
    ...
    int start = Integer.valueOf(page) * Integer.valueOf(size);
    String sql = "SELECT a.id, a.content, a.useraccount, a.groupId, a.loginType, date_format(a.createtime, '%Y-%m-%d %H:%i:%s') as createtime, (select username from tbl_reg_account where account = substring_index( substring_index(a.touseraccount, ':', 2  ), ':' , -1)) as tousername, a.type, a.parentId, b.username, b.growth_value, b.image as userimage from tbl_reply a left join tbl_reg_account b on a.useraccount = b.account where a.topicId = ? and a.status = 0 order by a.createtime desc limit " + start + ", " + size;
...
public void func357() throws Exception {
    ...
    public void func370() throws Exception {
        ...
        "select a.id, a.useraccount,  a.ssid, a.mac_addr, b.wifi_gps_address, date_format(a.createtime, '%Y-%m-%d %H:%i:%s') as createtime from tbl_topic a left join tbl_wifi_share_account_logs b on a.mac_addr = b.mac_address where a.useraccount = ? and a.status = 0 order by a.createtime desc limit " + start + ", "+ size +   
        " ) x left join (" + 
        "select account, username from tbl_reg_account where account = ?" +
        ") y on x.useraccount = y.account";
        
        List list = sysAccountDao.getData(sql, new String[]{useraccount, useraccount});
        if (list == null || list.isEmpty()){
            jsondata.put("resultcode", "0");
            jsondata.put("msg", "success");
            jsondata.put("function", "356");
    ...
    public void func375() throws Exception {
        ...
        if ("true".equals(isFirst)){
            ...
            if (StringUtils.isNotEmpty(groupId)){
                selectUserSQL = "select id, username, sex, province, growth_value, city, level, flowers, image as userimage from tbl_reg_account where groupid = '" + groupId + "' order by flowers desc limit 1";
            } else {                
                selectUserSQL = "select id, username, sex, province, growth_value, city, level, flowers, image as userimage from tbl_reg_account where account = '" + useraccount + "'";
            }
            
            List list = sysAccountDao.getData(selectUserSQL, new String[]{});
            ...
            List ls = sysAccountDao.getData("select groupid from tbl_reg_account where account=?", new String[]{useraccount});
            ...
            String sql = "";
            if (StringUtils.isNotEmpty(groupId)){
                sql = "select count(*) as num from tbl_wifi_share_account_logs where reg_account in (select account from tbl_reg_account where groupId = '" + groupId + "')";
            }else{
                sql = "select count(*) as num from tbl_wifi_share_account_logs where reg_account = '" + useraccount + "'";                
            }
            ...
            if (list != null && !list.isEmpty()){
                ...
                hotnum = mp.get("num") == null ? "0" : mp.get("num").toString();
            }   
            
            jsondata.put("sharehotspot", hotnum);
        }
        ...
        String userList = "";
        if (StringUtils.isNotEmpty(groupId)){            
            List ls= sysAccountDao.getData("select account from tbl_reg_account where  groupid = '" + groupId + "' and status = 0 ",new String[]{});
            if (null != ls && !ls.isEmpty()){
                for (int i = 0; i < ls.size(); i++){
                    Map mp = (Map)ls.get(i);
                    String acc = mp.get("account") == null ? "" : mp.get("account").toString();
        ...
        userList = StringUtils.isEmpty(userList) ? "'" + useraccount + "'" : userList;
        String sql = "SELECT id, ssid, useraccount, content, image, praiseQty, date_format(createtime, '%Y-%m-%d %H:%i:%s') as createtime from tbl_topic where useraccount in (" + userList + ") and status = 0 order by createtime desc limit " + start + ", "+ size;
        ...
        if (list != null && !list.isEmpty()){
            for (int i = 0; i < list.size(); i++){
                ...
                String ssid = map.get("ssid") == null || "".equals(map.get("ssid").toString()) ? "" : map.get("ssid").toString();
                String acc = map.get("useraccount") == null ? "" : map.get("useraccount").toString();
    ...
    public void func378() throws Exception{
        ...
        String useraccount = obj.getMyaccount();
        String groupId = "";
        String loginType = "";
        ...
        String userList = "";
        if (StringUtils.isNotEmpty(groupId)){            
            ls= sysAccountDao.getData("select account from tbl_reg_account where  groupid = '" + groupId + "' and status = 0 order by account",new String[]{});
            if (null != ls && !ls.isEmpty()){
                for (int i = 0; i < ls.size(); i++){
                    Map mp = (Map)ls.get(i);
                    String acc = mp.get("account") == null ? "" : mp.get("account").toString();
        ...
        String sql = "select u.image as userimage, u.id as userId,  u.username, b.topicId, b.content, date_format(b.createtime, '%Y-%m-%d %H:%i:%s') as createtime from tbl_reply b left join tbl_reg_account u on b.useraccount = u.account  where b.touseraccount in (" + userList + ") and b.useraccount not in (" + userList + ") and b.status = 0 order by createtime desc limit " + start + ", "+ size;
    ...
    public void func380() throws Exception{
        ...
        String currtime = Tools.date2String(new Date(), "yyyy-MM-dd HH:mm:ss");
        String account = obj.getMyuseraccount();
        if (account.contains(":") && account.split(":").length > 1){
        ...
        radius = radius * 1.1;
        com.aora.wifi.tools.LatitudeLontitudeUtil.Point[] locations = LatitudeLontitudeUtil.getRectangle4Point_v2(obj.getMylat(), obj.getMylng(), radius);
        
        //String sql = "select * from (SELECT a.id, a.image, a.mac_addr, a.content, a.useraccount, date_format(a.createtime, '%Y-%m-%d %H:%i:%s') as createtime, a.ssid, a.street, b.image as userimage, b.username FROM tbl_topic a  LEFT JOIN tbl_reg_account b on a.useraccount = b.account WHERE a.lat > " + locations[2].x + " AND a.lat < " + locations[0].x + " AND a.lng > " + locations[3].y + " AND a.lng < " + locations[1].y + " AND a.status = 0 order by a.createtime desc limit " + start + ", "+ size + " ) x group by mac_addr";
        String sql = "SELECT a.id, a.image, a.mac_addr, a.content, a.useraccount, a.groupId, a.loginType, date_format(a.createtime, '%Y-%m-%d %H:%i:%s') as createtime, a.ssid, a.street, b.image as userimage, b.username FROM tbl_topic a  LEFT JOIN tbl_reg_account b on a.useraccount = b.account WHERE a.lat > " + locations[2].x + " AND a.lat < " + locations[0].x + " AND a.lng > " + locations[3].y + " AND a.lng < " + locations[1].y + " AND a.status = 0 order by a.createtime desc limit " + start + ", "+ size;
        System.out.println("near topic sql:" + sql);
        List list = sysAccountDao.getData(sql, new String[]{});
        if (list == null || list.isEmpty()){
            jsondata.put("resultcode", "0");
            jsondata.put("msg", "success");
        ...
        for (int i = 0; i < list.size(); i++){
            Map map = (Map)list.get(i);
            String topicId = map.get("id") != null ? map.get("id").toString() : "";
            String mac_addr = map.get("mac_addr") != null ? map.get("mac_addr").toString() : "";
            String image = map.get("image") == null || "".equals(map.get("image").toString()) ? "" : PropertyInfo.getValue("access.domain", null) + request.getContextPath() + "/images/user/topic/" + map.get("image").toString();
            String content = map.get("content") != null ? map.get("content").toString() : "";
            String groupId =  map.get("groupId") != null && !"".equals(map.get("groupId").toString())? map.get("groupId").toString() : "";
            String loginType =  map.get("loginType") != null && !"".equals(map.get("loginType").toString())? map.get("loginType").toString() : "";
            ...
            String createtime = map.get("createtime") == null || "".equals(map.get("createtime").toString()) ? "" : map.get("createtime").toString();
            String ssid = map.get("ssid") == null || "".equals(map.get("ssid").toString()) ? "" : map.get("ssid").toString();
            String street = map.get("street") == null || "".equals(map.get("street").toString()) ? "" : map.get("street").toString();
            String userimage = map.get("userimage") == null || "".equals(map.get("userimage").toString()) ? "" : (map.get("userimage").toString().startsWith("http:") ? map.get("userimage").toString(): PropertyInfo.getValue("access.domain", null) + request.getContextPath() + "/images/user/" + map.get("userimage").toString());
            String username = map.get("username") != null ? map.get("username").toString() : "";
            ...
            item.put("ssid", ssid);
            item.put("mac", mac_addr);
            item.put("gps_addr", street);
            item.put("content", content);
            item.put("datetime", createtime);
            item.put("currtime", currtime);
            array.add(item);
        }
